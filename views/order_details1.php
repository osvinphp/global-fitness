...
/* <![CDATA[ */
var google_conversion_id = 951974390;
var google_conversion_label = "c6RcCLev3XYQ9vP3xQM";
...
.ANCHER_BLCK{color: #545454}
.REDCOLR{color: #cc0033;}
/*/////////////////////////----------POP UP CSS----------////////////////////////////////*/
/*/////////////////////////----------POP UP CSS----------////////////////////////////////*/
...
<section>
	<div class="container">
		...
		<div class="row thanku_page_penal">
          ...
          echo ucfirst($_SESSION['firstname']." ".$_SESSION['middlename']." ".$_SESSION['lastname']); ?>!</h2>
  			</div>
  		</div>
  		<?php
  		if(count($order)>0){
  		...
  		<div class="row thanku_page_penal">
  			<div class="col-md-12">
  				<h2 class="NEW_GREENCOLOR">Your order <?php echo $or->ID ;?> was succesfully placed. </h2>
  			</div>
  		</div>
  		<div class="row thanku_page_penal">
  			...
  			<div class="col-md-6 col-sm-6">
  				...
  				<div class="row">
	  				<div class="col-md-6 col-sm-6">
	  					...
	  					<p class="cust_info_text"><?php echo ucfirst($or->shippingcity)." , ".ucfirst($or->shippingstate)." , ".ucfirst($or->shippingzipcode);  ?></br ></p>
	  					<p class="cust_info_text"><?php echo ucfirst($or->shippingprimaryphone);  ?></br ></p>
	  					<?php  } ?>
	  					<p class="cust_info_text"><strong>Shipping Method</strong></p>
	  					<p class="cust_info_text">
	  						<?php if($or->shippinghome=="3"){
								echo "Shipping Domestically";
								}
	  				...
	  				<div class="col-md-6 col-sm-6">
	  					...
	  					<p class="cust_info_text"><?php echo ucfirst($or->billingcity)." , ".ucfirst($or->billingstates)." , ".ucfirst($or->billingzipcode);  ?></br ></p>
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="checkout_tittle">
                <div class="pull-left">
                    <h3>Order Confirmation Invoice</h3></div>
                <!-- <div class="pull-right">
                    <button class="btn btn-default EDITCHRCKOUT_NEW" name="submitpromo" type="submit">edit</button>
                </div> -->
            </div>
        </div>
        <!--end colm-->
    ...
    <?php
              ...
              $TaxPerProduct = explode(',,,,', $or->salestax);
              $totalcharges = 0;
              $totalprice = 0;
              $totalWeight= 0;
            ...
            <div class="row">
                ...
                if($live->IsWarranty ==1){
                 echo base_url('/public/assets/images/warranty_img_bottom.png');}else{
                  ...
                  ?>" alt="<?php echo $live->Name; ?>" title="<?php echo $live->MetaDetailPageTitleTag; ?>" class="img-responsive">
                    </div>
                </div>
            ...
            <div class="row CheCKOUT_NEW_PRICEBOX_penal NEW_BRD">
                <div class="col-md-6 col-sm-6 col-xs-4">
                    <div class="text-right">
                        <h5 class="H_TAG_NO_MGN">Qty <?php echo $piece[$i]; ?></h5>
                    </div>
                </div>
                    ...
                    <div class="row CheCKOUT_NEW_PRICEBOX">
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            ...
                            <div class="PRICE_BOX_NEW">
                                ...
                                <h5 class="HALF_PENAL H_TAG_NO_MGN"><?php print_r("$ ".number_format($TaxPerProduct[$i],2,".",","));?>  </h5>
                            </div>
                            <div class="PRICE_BOX_NEW">
                                <h5 class="HALF_PENAL H_TAG_NO_MGN">Shipping</h5>
                                <h5 id="shipTaxCharges1" class="HALF_PENAL H_TAG_NO_MGN"><?php echo "$". number_format($thischarges,2,".",","); ?></h5>
                            </div>
                            <div class="PRICE_BOX_NEW">
                                <h5 class="HALF_PENAL H_TAG_NO_MGN">Discount</h5>
                                <h5 id="shipTaxCharges1" class="HALF_PENAL H_TAG_NO_MGN REDCOLR">......</h5>
                            </div>
                        </div>
                    </div>
                </div>
...
<?php }else{ ?>
        ...
        <div class="col-md-6 col-sm-6 col-xs-12">
            <div class="row">
                <div class="col-md-5 col-sm-5 col-xs-4">
                    <div class="ChackOUT_NEW_CART_IMg">
                        <img src="<?php echo base_url().'/'.$live->ImageURL; ?>" alt="<?php echo $live->ProductName; ?>" title="<?php echo $live->MetaDetailPageTitleTag; ?>" class="img-responsive">
                    </div>
                </div>
                <div class="col-md-7 col-sm-7 col-xs-8">
                    <h5 class="PRODUCT_FONT H_TAG_NO_MGN"><a class="ANCHER_BLCK" href=""><?php if($live->Kingdom=="Cardio"){ ?>
...
'id': '<?php echo $order[0]->ID; ?>',                                                                           // Transaction ID. Required.
'affiliation': 'Global Fitness',                                                        // Affiliation or store name.
'revenue': '<?php echo $_SESSION["totalPrice"]; ?>',                                   //  Grand Total.
'shipping': '<?php echo $order[0]->allshipcharge; ?>',                                 //  Shipping.
'tax': '<?php echo $_SESSION["totalTaxOccur"]; ?>'                                     //  Tax.
});

ga('ecommerce:addItem', {
'id': '<?php echo $order[0]->ID; ?>',                                                                           // Transaction ID. Required.
'name': '<?php echo $_SESSION["fitness_details"][0]->ProductName; ?>',                 // Product name. Required.
'sku': '<?php echo $_SESSION["fitness_details"][0]->StockKeepingUnit; ?>',             // SKU/code.
'category': '<?php echo $_SESSION["fitness_details"][0]->CategoryName; ?>',            // Category or variation.
'price': '<?php echo $live->Price; ?>',                                                 // Unit price.
'quantity': '<?php $quant=explode(",,,,",$order[0]->piece);
               print_r(array_sum($quant)); ?>'                                           // Quantity.
});
ga('ecommerce:send');
</script>
		<?php 
            ...
            <div class="col-md-6 col-md-offset-6 col-sm-6 col-sm-offset-6 col-sm-12">
              <div class="row CheCKOUT_NEW_PRICEBOX_penal">
                  <div class="col-md-6 col-sm-6 col-xs-4">
                      <div class="Pull-left">
                          <!-- <div class="subscribe">
                              <div class="MINHHH H_TAG_NO_MGN NEW_BLUCOLOR" onclick="showspanPROMO();" id="pRMOCOTEXT">Promo Code?</div>
                              <form class="displaynone" id="PROMOCODENEW">
                                  <input type="text" placeholder="Promo Code" value="" size="18" id="promocodename" name="" class="form-control promo_COD">
                                  <a id="PromocodeApply" class="APPLYNOW NEW_BLUCOLOR">Apply</a>
                              </form>
                          </div> -->
                          <!--  <h5  class="H_TAG_NO_MGN NEW_BLUCOLOR DESKTOP_CHECKOUT"><a href="">Enter a Promotional Code</a></h5>
                          <h5 class="H_TAG_NO_MGN NEW_BLUCOLOR MOBILE_CHECKOUT PROMOCODE"><a href="">Promo Code?</a></h5> -->
                      </div>
                  </div>
                   <div class="col-md-6 col-sm-6 col-xs-8 Pull-right">
                      <div class="row CheCKOUT_NEW_PRICEBOX">
                          <div class="col-md-12 col-sm-12 col-xs-12">
                              <div class="PRICE_BOX_NEW">
                                  <h5 class="HALF_PENAL H_TAG_NO_MGN">Order Total</h5>
                                  <h5 id="mytotal" class="HALF_PENAL H_TAG_NO_MGN"><?php
                                  if($or->billingallpayment > $or->allmainpayment){
                                    $paymentOnBill = $or->billingallpayment;
                                  }else{
                                    $paymentOnBill = $or->allmainpayment;
                                  }
            echo "$".number_format($paymentOnBill,2,".",",");
           ?></h5>
                              </div>
                          </div>
                      </div>
			...
			<div class="col-md-12 col-sm-12">
			  	<div class="row align_right ">
					<div class="col-md-8 col-sm-7">
						<div class="continue_button marginnone"><a href="<?php echo base_url('fitness-equipment/used-treadmills'); ?>">Continue Shopping</a></div>
				    </div>
			  		<div class="col-md-4 col-sm-5">
			  		<p class="phone_contact">Questions? Call 1-888-991-9991</p>
					</div>
				</div>
			</div>
		</div>
