...

{
  ...
  public  function home()
  {
    ...
    $data['CarouselHeading'] = $this->Site_model->SelectData('zHomePageCarousel');
    $data['CarouselData'] = $this->Site_model->SelectData('zHomePageCarouselData');
    $data['ColappseDiv'] = $this->Site_model->SelectData('zHomePageCollapsibleDiv');
    $data['BlurbData'] = $this->Site_model->SelectData('zHomePageBlurbData');
    $data['FooterLinkData'] = $this->Site_model->SelectData('zHomePageFooterLinkData ');
    $data['VideoData'] = $this->Site_model->SelectData('zHomePageVideoData');
    $data['PromotionalData'] = $this->Site_model->PromotionalData('zHomePagePromotionalData');

    // echo "<pre>"; print_r($data['PromotionalData']); die;
    ...
    $data['description'] = 'Buy used gym equipment or browse the largest inventory of used exercise equipment and refurbished gym equipment for your gym or home. We ship all commercial brands of second hand gym equipment worldwide from our factory in Los Angeles. We are the world’s largest wholesaler of used fitness equipment.';
    $data['keywords'] = 'used gym equipment, used exercise equipment, refurbished gym equipment, second hand gym equipment, used fitness equipment, commercial fitness equipment, fitness equipment, used treadmill, used life fitness treadmill, precor elliptical, elliptical trainer, used precor elliptical, global fitness, globalfitness';
    $data['ptype'] = "0";
  ...
  function product($id)
  {
   ...
   if (isset($_POST['message_SellProduct'])) {
    ...
    if ($id != "") {
      ...
      $video  = $this->Site_model->GetVideo($result[0]->ListID);
      $slideImages = $this->Site_model->selectwithwhere('TOP 3 *','AdditionalImagesByMPN', 'MPN' ,$result[0]->MPN);  
      $ProductDescription = $this->Site_model->selectwithwhere('*','ProductionDetails', 'IDKey' ,$result[0]->IDKey);  
      if(!empty($ProductDescription)){
        $data['ProductionDetails'] = $ProductDescription;
      }
